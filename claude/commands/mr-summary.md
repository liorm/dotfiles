...
- Include relevant issue numbers or references if applicable in the title. Check the branch name to understand the issue number.
...
**Example:**
```
# fix(XXX-123): Did something good
...
### Key change 1
- **Enhanced totalRuns tracking**: Added proper increment logic in `PipelineExecutor.executePipelineFunction()` to track all execution attempts regardless of reason (retry, crash recovery, input processing)
- **Separated retry vs crash recovery**: Distinguished between user-initiated retries (`retryCount`) and system-level crash recovery executions
- **Improved logging context**: Enhanced function execution logs to include accurate `totalRuns` values for better debugging
